...
class TestVerifier:
    ...
    def load_manifest(self) -> List[Dict]:
        ...
        if not self.manifest_path.exists():
            raise TestVerificationError(
                "manifest.json is missing. Please run test_data_generator.py first."
            )
        ...
        try:
            ...
            if not isinstance(manifest, list) or not manifest:
                raise TestVerificationError(
                    "Invalid manifest format: expected non-empty list"
                )
    ...
    def prepare_test_files(self, manifest: List[Dict]) -> None:
        ...
        if self.errors:
            raise TestVerificationError(
                f"Failed to prepare {len(self.errors)} test file(s)"
            )
    ...
    def run_file_organization(self) -> None:
        ...
        handler = DocumentHandler(
            str(self.test_docs_path), enable_organization, categories, variables
        )
    ...
    def verify_results(self, manifest: List[Dict]) -> None:
        ...
        for item in manifest:
            ...
            if not expected_path.exists():
                ...
                print(
                    f"  ✅ Found: {item['expected_filename']} in {item['expected_category']}"
                )
        ...
        if unexpected_files:
            self.errors.append(
                f"❌ UNEXPECTED: Found {len(unexpected_files)} unexpected file(s) in category directories"
            )
        ...
        if leftover_files:
            self.errors.append(
                f"❌ LEFTOVER: Found {len(leftover_files)} unprocessed file(s) in root directory"
            )
